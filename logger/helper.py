...
import torch
import csv
from logger import vis_utils
from logger.metrics import Result

fieldnames = [
    'iteration', 'rmse', 'photo', 'mae', 'irmse', 'imae', 'mse', 'absrel', 'lg10',
...
def get_folder_name(args):
    ...
    return os.path.join(args.output_directory,
            '{}_pose_{}_sup_{}'.
            format(args.pose_src, args.objective, current_time
                ))
...
def multiscale(img, avg=True):
    if avg:
        pool = torch.nn.AvgPool2d(kernel_size=2, stride=2).cuda()
    else:
        pool = torch.nn.MaxPool2d(kernel_size=2, stride=2).cuda()
    img1 = pool(img)  # 1/2
    img2 = pool(img1)  # 1/4
    img3 = pool(img2)  # 1/8
    return img3, img2, img1, img
